...
void perceptionDataCallback(const std_msgs::Float32MultiArray::ConstPtr& msg)
{
    ...
    perceptionPoint = {centX, centY, height / 2};
...
int main(int argc, char** argv) 
{
    ...
    ros::Subscriber sub = nh.subscribe("/perception_topic", 1, perceptionDataCallback); // Replace with the actual topic name
    ...
    if (perceptionDataReceived && perceptionPoint.x >= 0.01813680100817779)
    {
        ...
        Point initialWP = perceptionPoint;
        ...
        // Output final waypoint with orientation
        std::cout << "Final Waypoint: (" << finalWP.x << ", " << finalWP.y << ", " << finalWP.z << "), Orientation: (1, 0, 0, 0)" << std::endl;
    }
    else
    {
